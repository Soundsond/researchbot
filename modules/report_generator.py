...
class ReportGenerator:
    ...
    async def _get_recommendation(self, by_competitor: dict, influencers: list) -> str:
        ...
        for competitor, promos in by_competitor.items():
            ...
            for p in top:
                summary += (f"- {competitor}: @{p['author_handle']} "
                           f"({p['author_followers']:,} followers), "
                           f"{p['likes']} likes, {p['retweets']} RT, "
                           f"type: {p['promo_type']}, sentiment: {p['sentiment']}\n")

        try:
            message = self.client.messages.create(
                model="claude-haiku-4-5-20251001",
                max_tokens=300,
                system=("You are a crypto casino marketing analyst. "
                        "Based on competitor promo data, give ONE concise recommendation "
                        "in Russian (2-3 sentences max) on where to place ads. "
                        "Be specific about which influencer or community to target and why."),
    ...
    def _build_report(self, by_competitor: dict, influencers: list, recommendation: str) -> str:
        ...
        for competitor, promos in by_competitor.items():
            ...
            for p in top_promos:
                promo_type = p.get("promo_type", "other").replace("_", " ").title()
                lines.append(f"\n• {competitor.capitalize()} — {promo_type}")
        ...
        if influencers:
            ...
            for i, inf in enumerate(influencers, 1):
                lines.append(
                    f"{i}. @{inf['handle']} — {inf['followers']:,} fol | "
